...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{route('admin.dashboard.index')}}" class="flex items-center p-2 text-gray-900 rounded-lg {{($title === 'Dashboard') ? 'bg-secondary' : 'bg-white' }}">
               <svg class="w-5 h-5 text-gray-500 transition duration-75 {{($title === 'Dashboard') ? 'text-gray-900' : ' text-gray-500' }}" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
         ...
         <li>
            <a href="/admin/data" class="flex items-center p-2 text-gray-900 rounded-lg {{($title === 'Data BBM' || $title ===  'Pertamina' || $title === 'Shell' || $title === 'Vivo' || $title === 'BP' || $title ===  'Edit BBM' || $title ===  'Tambah Data') ? 'text-gray-900 bg-secondary' : ' text-gray-500' }}"">
               ...
               <span class="flex-1 ms-3 whitespace-nowrap">Data BBM</span>
            </a>
         </li>
